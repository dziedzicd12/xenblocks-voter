...
fn generate_keypair(file_path: &str) -> Result<Keypair, Box<dyn Error>> {
    if let Ok(mut file) = OpenOptions::new().read(true).open(file_path) {
        ...
        println!("\n{} now has your private key, delete it to generate new one\n", file_path);

        // Serialize the private key to JSON and write it to the specified id.json
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let matches = Command::new("My App")
        ...
        .arg(
            Arg::new("id")
                .index(1)
                .help("The ID to specify which id.json to use")
                .required(true),
        )
        .get_matches();
    ...
    let file_path = format!("id{}.json", id);
